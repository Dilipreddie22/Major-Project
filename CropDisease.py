...
main = Tk()
main.title("Plant Disease Detection and Classification with Pesticide Suggestion for e-Agriculture Applications")
...
pesticide = ['Seed Treatment with Hot Water', 'No Pesticide is required', 'Proxanil', 'No Pesticide is required', 'Proxanil', 'Insecticidial Soap and Horticultural Oil',
          'Insecticidial Soap and Horticultural Oil', 'Tomato__Tomato_YellowLeaf__Curl_Virus', 'Insecticidial Soap and Horticultural Oil', ' Bonide tomato', 'No Pesticide is required',
          'Copper based fungicide', 'Applying fungicides', 'Mancozeb', 'Insecticidial Soap and Horticultural Oil']
...
def cnnModel():
    ...
    if os.path.exists('model/model.json'):
        ...
        acc = accuracy['accuracy']
        acc = acc[9] * 100
        text.insert(END,"Transfer Learning Model Accuracy = "+str(acc))
...
def predict():
    ...
    cv2.putText(img, 'Suggested pesticide is : '+pesticide[predict], (10, 250),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 0, 255), 2)
...
font = ('times', 15, 'bold')
title = Label(main, text='Plant Disease Detection and Classification with Pesticide Suggestion for e-Agriculture Applications')
